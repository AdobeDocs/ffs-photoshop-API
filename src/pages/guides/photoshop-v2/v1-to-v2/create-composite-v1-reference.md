...
|---|---|---|
| `width` | `width` | 1–32000 (unit set by `image.unit`; converted value ≤ 32000 px) |
| `height` | `height` | 1–32000 (unit set by `image.unit`; converted value ≤ 32000 px) |
...
| *(not in V1)* | `unit: "pixels_unit"` / `"points_unit"` | New: unit for `width`/`height`; default `"pixels_unit"` |
...
| *(not in V1)* | `transform.dimension.unit` | New: `"pixels_unit"` (default) or `"points_unit"` |
...
| `mask.linked` | `pixelMask.isLinked` | Renamed + prefix changed |
| `mask.enabled` | `pixelMask.isEnabled` | Renamed + prefix changed |
| `mask.input: {href, storage}` | `pixelMask.source.url` | Breaking: restructured |
...
> **`type` is required on every `edits.layers[]` entry.** V2 does not infer the layer type. Omitting `type` causes a validation error even for simple edits like visibility or name changes.
>
> Valid types: `"layer"`, `"text_layer"`, `"adjustment_layer"`, `"smart_object_layer"`, `"group_layer"`, `"solid_color_layer"`.
>
> **V1 (no type needed — inferred):** `{"name": "My Layer", "isVisible": false}`
>
> **V2 (type required):** `{"type": "layer", "name": "My Layer", "isVisible": false}`
...
| V1 | V2 |
|---|---|
| `type: "fillLayer"` | `type: "solid_color_layer"` |
| `fill.solidColor.rgb.red` | `fill.solidColor.rgb.red` (unchanged) |
| `fill.solidColor.rgb.green` | `fill.solidColor.rgb.green` (unchanged) |
| `fill.solidColor.rgb.blue` | `fill.solidColor.rgb.blue` (unchanged) |

> **Note:** Only the layer type name changes. V1 and V2 both use `fill.solidColor.rgb.{red, green, blue}` — the `rgb` wrapper is required in both. Do not remove it.
>
> **V1 and V2 (identical structure):** `{"fill": {"solidColor": {"rgb": {"red": 255, "green": 0, "blue": 0}}}}`
>
> Error when `rgb` wrapper is omitted: `"edits.layers[1].fill.solidColor.rgb: rgb is required"`.
...
- **Mask:** `mask.input` → `pixelMask.source.url`; `mask.linked` → `pixelMask.isLinked`; `mask.enabled` → `pixelMask.isEnabled`
...
> **V1 `outputs[].layers[]` with `visible` field:** V1 `renditionCreate`/`documentOperations` payloads sometimes used `outputs[].layers[]` entries with `{id, visible: true}` to select which layers appear in the composite export. Map these directly to V2 `outputs[].layers[]` as `[{id: N}, ...]` — this triggers multi-layer composite export. Do **not** move them to `edits.layers[]` (that path requires `type` + `operation` and performs layer edits, not export selection).
