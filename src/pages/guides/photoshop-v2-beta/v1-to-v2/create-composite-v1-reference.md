...
**`type` is required on every `edits.layers[]` entry.** V2 does not infer the layer type. Omitting `type` causes a validation error even for simple edits like visibility or name changes. Valid types: `"layer"`, `"text_layer"`, `"adjustment_layer"`, `"smart_object_layer"`, `"group_layer"`, `"solid_color_layer"`.
