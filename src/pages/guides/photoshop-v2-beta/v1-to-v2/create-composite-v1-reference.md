...
| V1 | V2 |
|---|---|
| `fills[0].solidColor.rgb.red` | `fill.solidColor.red` |
| `fills[0].solidColor.rgb.green` | `fill.solidColor.green` |
| `fills[0].solidColor.rgb.blue` | `fill.solidColor.blue` |

> **Breaking change:** V1 used a `fills` **array** (plural) with a nested `solidColor.rgb` wrapper. V2 uses a `fill` **object** (singular) with `solidColor` directly containing the color components — the `rgb` wrapper is removed.
>
